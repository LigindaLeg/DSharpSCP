    ...
    public static class ServerEvents
    {
        public static void CommandExecuted(CommandExecutedEventArgs ev)
        {
            if (Player.Get(ev.Sender) == null || Player.Get(ev.Sender).IsNpc|| Player.Get(ev.Sender).IsHost)
                return;
            _ = SendLog(Main.Instance.Config.CommandLog.Replace("{player}", Player.Get(ev.Sender).Nickname).Replace("{playerid}", Player.Get(ev.Sender).UserId).Replace("{command}", ev.Command.Command  + ev.Arguments), Main.Instance.Config.CommandLogChannel);
        }
